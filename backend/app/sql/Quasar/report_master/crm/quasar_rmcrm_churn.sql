-- Report Master / CRM: Churn (taxa: saíram ÷ ativos do mês anterior, fração p/ formato 0,00%)
...
WITH UsuariosAtivos AS (
    ...
    FROM (
        ...
        WHERE date_agg >= DATEADD(month, -1, @data_ini) AND date_agg < @data_fim_exclusive
        UNION ALL
...
SELECT
    CAST(
        SUM(CASE WHEN u.mes_anterior = 1 AND (u.mes_atual = 0 OR ISNULL(f.ftd_atual, 0) = 1) THEN 1.0 ELSE 0 END)
        / NULLIF(SUM(CASE WHEN u.mes_anterior = 1 THEN 1.0 ELSE 0 END), 0)
    AS DECIMAL(10,4)) AS [Churn]
