...
-- CTE: Prepara tabela de gerentes (Join por Username)
    ...
    FROM
        (SELECT COUNT(DISTINCT ftd.user_id) AS FTD_QTD
         ...
         WHERE ftd.ftd_date >= @data_ini1 AND ftd.ftd_date < @data_fim_exclusive1 
           AND am.channel_type = 'Tráfego externo') ftd_counts
    LEFT JOIN (
...
SELECT
    'Tráfego Externo' AS Affiliate_Manager,
    ...
    -- Cost e Cost_Percent
    COALESCE(p2.FTD_QTD, 0) * 200 AS Cost,
    CAST(((COALESCE(p2.FTD_QTD, 0) * 200.0 - COALESCE(p1.FTD_QTD, 0) * 200.0) * 100.0 / NULLIF(COALESCE(p1.FTD_QTD, 0) * 200.0, 0)) AS DECIMAL(10,2)) AS Cost_Percent
