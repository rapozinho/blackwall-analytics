...
dataset_period AS (
    ...
    FROM
        (SELECT COUNT(DISTINCT ftd.user_id) AS FTD_QTD
         ...
         WHERE ftd.ftd_date >= @data_ini1 AND ftd.ftd_date < @data_fim_exclusive1 
           AND acq.acquisition_channel IN ('ZP0902CAMP26', 'Peakline Media', 'LiveOdds', 'Basalt', 'Ravena', 'Ascend', 'Polaris', 'Vertex', 'ParceiroAlfaAff', 'Somma Ads')) ftd_counts
    LEFT JOIN (
        ...
        FROM (
            ...
            INNER JOIN acquisitions_agg acq WITH(NOLOCK) ON ftd.user_id = acq.user_id
            WHERE ftd.ftd_date >= @data_ini1 AND ftd.ftd_date < @data_fim_exclusive1 
              AND acq.acquisition_channel IN ('ZP0902CAMP26', 'Peakline Media', 'LiveOdds', 'Basalt', 'Ravena', 'Ascend', 'Polaris', 'Vertex', 'ParceiroAlfaAff', 'Somma Ads')
    ...
    LEFT JOIN (
        ...
        FROM (
            ...
            WHERE acq.acquisition_channel IN ('ZP0902CAMP26', 'Peakline Media', 'LiveOdds', 'Basalt', 'Ravena', 'Ascend', 'Polaris', 'Vertex', 'ParceiroAlfaAff', 'Somma Ads')
        ) AS Acquired_Users
