...
UserAffiliateManager AS (
    ...
    FROM (
        SELECT
            SubQuery.*,
            -- Deduplica para 1 acquisition por usuario. Sem isso, um user com varios
            -- affiliate_name em acquisitions_agg cai em varios grupos e a SUM de FTDs
            -- infla (ex.: 1149 no monitoring vs 286 no BPA1, que usa COUNT(DISTINCT user_id)).
            ROW_NUMBER() OVER (
                PARTITION BY SubQuery.user_id, SubQuery.periodo
                ORDER BY SubQuery.affiliate_id DESC
            ) AS rn_user
        FROM (
            SELECT DISTINCT
                ftd.user_id,
        ...
        -- Filtra manager valido ANTES do ranking, para nao descartar o usuario
        -- caso o acq de maior affiliate_id tenha manager nulo.
        WHERE SubQuery.affiliate_manager IS NOT NULL
    ) AS Ranked
    WHERE rn_user = 1
),
UserGGR AS (
