...
SELECT
    'Grand Total',
    p2.FTD_QTD,
    CAST((p2.FTD_QTD - p1.FTD_QTD) * 100.0 / NULLIF(p1.FTD_QTD, 0) AS DECIMAL(18,2)) AS FTD_Percent,

    p2.dep_current_cohort,
    CAST((p2.dep_current_cohort - p1.dep_current_cohort) * 100.0 / NULLIF(p1.dep_current_cohort, 0) AS DECIMAL(18,2)) AS dep_current_Percent,

    p2.dep_all_cohort,
    CAST((p2.dep_all_cohort - p1.dep_all_cohort) * 100.0 / NULLIF(p1.dep_all_cohort, 0) AS DECIMAL(18,2)) AS dep_all_Percent,

    p2.GGR_current_cohort,
    CAST((p2.GGR_current_cohort - p1.GGR_current_cohort) * 100.0 / NULLIF(p1.GGR_current_cohort, 0) AS DECIMAL(18,2)) AS GGR_current_Percent,
    
    p2.GGR_all_cohort,
    CAST((p2.GGR_all_cohort - p1.GGR_all_cohort) * 100.0 / NULLIF(p1.GGR_all_cohort, 0) AS DECIMAL(18,2)) AS GGR_all_Percent
