    ...
    FROM
        (SELECT COUNT(DISTINCT ftd.user_id) AS FTD_QTD
         ...
         WHERE ftd.ftd_date >= @data_ini1 AND ftd.ftd_date < @data_fim_exclusive1
           AND aff.affiliate_manager IN ('GoogleAds', 'GoogleAdsNOVO', 'MetaAds', 'MetaAdsNOVO', 'TráfegoGeral')) ftd_counts

    LEFT JOIN (
        SELECT CAST(SUM(ug.GGR_Total) AS DECIMAL(10,2)) AS GGR_current_cohort
        FROM acquisitions_agg r
        LEFT JOIN affiliates_agg aff ON aff.Affiliate_Id = r.Affiliate_Id
        ...
        WHERE ug.Date_Agg >= @data_ini1 AND ug.Date_Agg < @data_fim_exclusive1
          AND aff.affiliate_manager IN ('GoogleAds', 'GoogleAdsNOVO', 'MetaAds', 'MetaAdsNOVO', 'TráfegoGeral')
    ) ggr_current ON 1=1
